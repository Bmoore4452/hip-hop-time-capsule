...
The DJ Scipio (`djScipioAnswers.ts`) and Anita Scipio (`anitaScipioAnswers.ts`) data files share a similar shape: an array of page objects keyed by `pageNumber`, each holding `answers[]` of `{ questionNumber, questionText, answerText, qrCodes?, ripNote? }`. Anita pages additionally have a `pageType` of `"answers" | "continued"`, and an answer block may set `continued: true` to render a "Continued..." carry-over block (optionally QR-codes-only) instead of a numbered question. To edit Q&A content, edit these data files — no component changes needed.

All QR-code `url` values across the app (Foreword page 8, Introduction page 18, DJ Scipio/Anita answer pages, Yankee concert pages, back matter) are the **exact payloads decoded from the printed book's QR codes** — do not replace them with search-query or guessed URLs. Where a printed payload is not a bare URL, `YankeeQR.qrValue` holds the exact payload for the rendered QR while `url` holds the tappable link. A few printed QRs point to since-deleted YouTube videos (pages 95, 159, 191, 193, 200, 201) — kept faithful to print. Pages 8 and 18 are listed in `isQRPage` in `EbookReader.tsx` so the center nav tap zone doesn't swallow QR taps.
